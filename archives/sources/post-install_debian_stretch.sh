...
test_se3()
{
    if [ -n "${ip_se3}" ]
    then
        #TEST_CLIENT=$(ip -4 -o addr show | grep  "$ip_se3")
        if [ -e "/var/www/se3" ]
        then
...
activer_gdm()
{
    ...
    # bug de gdm3 ? Ou commande d'activation inconnue ? [TODO]
    # lancement du script d'initialisation en attendant…
    bash /etc/gdm3/Init/Default
}
