...
# Script lancé en fin d'installation preseed ubuntu
# pour finaliser de la config .
#
# 
# lastupdate 5-06-2014 initab
...
echo "Installation des paquets définis dans mesapplis-ubuntu.txt"
sleep 2
apt-get -q update
# aptitude -y full-upgrade
apt-get install -y tofrodos
fromdos /root/bin/mesapplis-ubuntu.txt
for i in $(cat /root/bin/mesapplis-ubuntu.txt)
do
...
echo -e "${jaune}"
echo -e "==========================================="
echo -e "Fin de l'installation des paquets mesapplis-ubuntu"
echo -e "===========================================${neutre}"
...
# Suppression autologin
sed 's|-a root||' -i /etc/init/tty1.conf 
